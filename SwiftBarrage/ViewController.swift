...
class ViewController: UIViewController {
    ...
    func addMixedTextAndImage() {
        ...
        if let path = Bundle.main.path(forResource: "demoIcon@2x", ofType: "png") {
            ...
            if let data = try? Data(contentsOf: url) {
                ...
                attributedString.append(NSAttributedString(string: "SwiftBarrage", attributes: [NSAttributedStringKey.foregroundColor:UIColor.red]))
                ...
                attributedString.addAttribute(NSAttributedStringKey.strokeWidth, value: NSNumber(value: -1), range: NSRange(location: 0, length: attributedString.length))
                attributedString.addAttribute(NSAttributedStringKey.strokeColor, value: UIColor.white, range: NSRange(location: 0, length: attributedString.length))
                attributedString.addAttribute(NSAttributedStringKey.font, value: UIFont.boldSystemFont(ofSize: 25), range: NSRange(location: 0, length: attributedString.length))
    ...
    func addNoble() {
        ...
        let attributedString = NSMutableAttributedString(string: "~SwiftBarrage~全民直播~荣誉出品~")
